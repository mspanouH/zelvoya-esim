...
export function canAccessOrder(order, user, token) {
  // A valid token is a claim on its own. Checking the session first would mean
  // signing in with a different address revokes access to a link you hold.
  if (verifyOrderAccess(order.id, token)) return true;
